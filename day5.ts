...
allSeatIds.forEach((x, i) => {
    if (allSeatIds[i-1]){
        ...
        if (currentSeatId - prevSeatId > 1) console.log(`previousSeat:${prevSeatId}, currentSeat:${currentSeatId}`)
    }
})
...
//e.g. 'FBBFFFB'
function rowCodeParser(input: string): number{
    let front = 1
    let back = 128
...
//e.g. 'LRR'
function seatCodeParser(input: string): number{
    let left = 1
    let right = 8
